...
@implementation UIView (GKLFadeAnimation)
...
- (NSTimeInterval)intervalDuration
{
    ...
    if (!count) {
        objc_setAssociatedObject(self, kIntervalDurationKey, @(0.006), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
- (NSTimeInterval)fadeAnimationDuration
{
    ...
    if (!count) {
        objc_setAssociatedObject(self, kAnimationDurationKey, @(0.1), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
...
- (void)setHorizontalCount: (NSInteger)horizontalCount
{
    ...
    horizontalCount = MAX(16,horizontalCount);
